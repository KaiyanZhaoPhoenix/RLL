...
def initialize_wandb(args):
    ...
    run = wandb.init(
        project="gen",
        config=vars(args),
        name=f"ebp_dqn_{args.env}_{args.seed}_{int(time.time())}",
        monitor_gym=True,
        save_code=True,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='EBP-DQN for Atari')
    ...
    parser.add_argument("--w_state", type=float, default=1.0,
                       help="Weight for state change energy component")
    parser.add_argument("--w_reward", type=float, default=1.0,
                       help="Weight for reward energy component")
    parser.add_argument("--w_action", type=float, default=0.5,
                       help="Weight for action energy component")
